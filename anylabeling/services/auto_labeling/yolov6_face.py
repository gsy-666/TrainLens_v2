...
class YOLOv6Face(Model):
        ...
        if not model_abs_path or not os.path.isfile(model_abs_path):
            raise FileNotFoundError(
                QCoreApplication.translate(
                    "Model", "Could not download or initialize YOLOv6Face model."
                )
            )
    ...
    def post_process(self, img_src, img_processed, outputs):
        ...
        """
        det = self.non_max_suppression_face(outputs)[0]

        output_infos = []
        if len(det):
            det[:, :4], det[:, -10:] = self.rescale(img_processed.shape[2:], det[:, :4], det[:, -10:], img_src.shape)
    ...
    def predict_shapes(self, image, image_path=None):
        ...
        for info in infos:
            ...
            num_kps = len(info["kps"])
            for i in range(num_kps):
                point_shape = Shape(label=self.kps_classes[i], shape_type="point", flags={})
                point_shape.add_point(QtCore.QPointF(info["kps"][i][0], info["kps"][i][1]))
    ...
    def rescale(ori_shape, boxes, lmdks, target_shape):
        ...
        lmdks[:, :10] /= ratio
        lmdks[:, 0] = np.clip(lmdks[:, 0], 0, target_shape[1])
        lmdks[:, 1] = np.clip(lmdks[:, 1], 0, target_shape[0])
        lmdks[:, 2] = np.clip(lmdks[:, 2], 0, target_shape[1])
        lmdks[:, 3] = np.clip(lmdks[:, 3], 0, target_shape[0])
        lmdks[:, 4] = np.clip(lmdks[:, 4], 0, target_shape[1])
        lmdks[:, 5] = np.clip(lmdks[:, 5], 0, target_shape[0])
        lmdks[:, 6] = np.clip(lmdks[:, 6], 0, target_shape[1])
        lmdks[:, 7] = np.clip(lmdks[:, 7], 0, target_shape[0])
        lmdks[:, 8] = np.clip(lmdks[:, 8], 0, target_shape[1])
        lmdks[:, 9] = np.clip(lmdks[:, 9], 0, target_shape[0])

        return np.round(boxes), np.round(lmdks)

    def non_max_suppression_face(self, prediction, classes=None, agnostic=False, multi_label=False, max_det=1000):
        ...
        pred_candidates = np.logical_and(prediction[..., 14] > conf_thres, np.max(prediction[..., 15:], axis=-1) > conf_thres)
        ...
        for img_idx, x in enumerate(prediction):  # image index, image inference
            ...
            if multi_label:
                box_idx, class_idx = np.nonzero(x[:, 15:] > conf_thres).T
                x = np.concatenate((box[box_idx], x[box_idx, class_idx + 15, None], class_idx[:, None].astype(np.float32), x[box_idx, 4:14]), 1)
            else:
                conf, class_idx = np.max(x[:, 15:], axis=1, keepdims=True), np.argmax(x[:, 15:], axis=1, keepdims=True)
                x = np.concatenate((box, conf, class_idx.astype(np.float32), x[:, 4:14]), 1)[conf.ravel() > conf_thres]

            # Filter by class, only keep boxes whose category is in classes.
            if classes is not None:
                x = x[np.isin(x[:, 5:6], np.asarray(classes)).any(1)]
