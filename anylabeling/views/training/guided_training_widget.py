...
class GuidedTrainingWidget(QWidget):
    def __init__(
        ...
        # Store callbacks for image loading
        self._open_folder_callback = open_folder_callback
        self._get_image_list_callback = get_image_list_callback
    ...
    def load_images(self):
        """Load images from host application

        Uses callbacks to decouple from specific parent implementation.
        Falls back to safe no-op if callbacks not provided.
        """
        if callable(self._open_folder_callback):
            # Call host's open folder dialog
            self._open_folder_callback()

            # Refresh image list from host
            if callable(self._get_image_list_callback):
                self.image_list = self._get_image_list_callback()
            ...
            # No callback provided - show informative message
            QMessageBox.information(
                self,
                self.tr("Load Images"),
                self.tr("Image loading is not available in standalone mode.\n"
                       "Please provide images through the constructor or use "
